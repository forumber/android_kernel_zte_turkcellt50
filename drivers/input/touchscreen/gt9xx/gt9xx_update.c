...
extern int gtp_i2c_read_dbl_check(struct i2c_client *client,
...
extern  int gtp_send_cfg(struct goodix_ts_data *ts);
								...
								u8 *path)
{
	...
	if (path) {
		...
		/* update config */
		update_msg.cfg_file = filp_open(CONFIG_FILE_PATH_1,
							O_RDONLY, 0);

		if (IS_ERR(update_msg.cfg_file)) {
			pr_err("%s is unavailable", CONFIG_FILE_PATH_1);
		} else {
			pr_err("Update Config File: %s", CONFIG_FILE_PATH_1);
		...
		*4 + FW_DSP_ISP_LENGTH+FW_DSP_LENGTH + FW_BOOT_LENGTH)) {
		...
		update_msg.file = filp_open(UPDATE_FILE_PATH_2, O_CREAT |
								O_RDWR, 0666);
		if ((IS_ERR(update_msg.file))) {
		...
		for (i = 0; i < GUP_SEARCH_FILE_TIMES; i++) {
			...
			if (!(got_file_flag&0x0F)) {
				update_msg.file =open(search_update_path,
						O_RDONLY, 0);
				if (!IS_ERR(update_msg.file)) {
			...
			if (!(got_file_flag & 0xF0)) {
				update_msg.cfg_file = filp_open(search_cfg_path,
						O_RDONLY, 0);
				if (!IS_ERR(update_msg.cfg_file)) {
	...
	for (i = 0; i < FW_SECTION_LENGTH * 4 + FW_DSP_ISP_LENGTH +
			FW_DSP_LENGTH + FW_BOOT_LENGTH; i =i+ 2) {
