...
static const u16 touch_key_array[] = {KEY_BACK, KEY_HOMEPAGE,KEY_MENU };/*[BUGFIX]-MOD by TCTNB.XQJ,09/18/2013,FR-514195,GT915 tp MIATA development,i2c*/
#define GTP_MAX_KEY_NUM  (sizeof(touch_key_array)/sizeof(touch_key_array[0]))

#if GTP_DEBUG_ON
static const int  key_codes[] = {
	KEY_BACK, KEY_HOMEPAGE,KEY_MENU /*[BUGFIX]-MOD by TCTNB.XQJ,09/18/2013,FR-514195,GT915 tp MIATA development,i2c*/

};
static const char *const key_names[] = {
	"Key_Menu","KEY_HOMEPAGE", "Key_Back"/*[BUGFIX]-MOD by TCTNB.XQJ,09/18/2013,FR-514195,GT915 tp MIATA development,i2c*/
};
#endif
#endif
 ...
 int gtp_send_cfg(struct goodix_ts_data *ts)
{
	...
	if (ts->fixed_cfg) {
		...
		{
		   ret = gtp_i2c_write(ts->client,config,
		...
		int w)
{
	...
	pr_err("ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
}
...
static s32 gtp_get_info(struct goodix_ts_data *ts)
{
    ...
    dev_err(&ts->client->dev,"X_MAX = %d, Y_MAX = %d, TRIGGER = 0x%02x",
...
static int gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	ret = gtp_i2c_read_dbl_check(ts->client, 0x41E4, opr_buf, 1);
		...
		if (SUCCESS == ret)
		{
		    if (sensor_id >= 0x06)
		    {
			//dev_err("Invalid sensor_id(0x%02X), No Config Sent!", sensor_id);
			ts->pnl_init_error = 1;
			{
			    gtp_get_info(ts);
		    }
		...
		{
		    //dev_err("Failed to get sensor_id, No config sent!");
		    ts->pnl_init_error = 1;
		    return -1;
		}
		pr_err("Sensor_ID: %d", sensor_id);
	}
	ts->gtp_cfg_len = cfg_info_len[sensor_id];
	    ...
	    {
		//dev_err("Config Group%d is INVALID CONFIG GROUP(Len: %d)! NO Config Sent! You need to check you header file CFG_GROUP section!", sensor_id+1, ts->gtp_cfg_len);
		ts->pnl_init_error = 1;
		return -1;
	}
	...
	ts->gtp_cfg_len = GTP_CONFIG_MAX_LENGTH;
	    ...
	    if (ret < 0)
	    {
		//dev_err("Read Config Failed, Using Default Resolution & INT Trigger!");
...
pr_err( "GTP goodix_ts_init ret =%d\n",ret);
	return ret;
}
