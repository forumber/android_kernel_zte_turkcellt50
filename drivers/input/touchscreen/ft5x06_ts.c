	  ...
	  void *data)
{
	...
	err = ft5x06_i2c_read(update_client, &reg_addr, 1, &reg_value, 1);
	buf = reg_value;
	...
	err = ft5x06_i2c_read(update_client, &reg_addr, 1, &buf, 1);	
			   ...
			   const struct i2c_device_id *id)
{
	...
	u8 reg_addr;
	int err;
	struct proc_dir_entry *dir, *refresh;	//add for test ADB update
	
	dev_err(&client->dev, "ft5x06 entry\n");
