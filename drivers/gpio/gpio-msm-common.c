...
//copy from gpio-msm-v2.c
void set_gpio_bits(unsigned n, void __iomem *reg)
{
	__raw_writel(__raw_readl(reg) | n, reg);
}

void clr_gpio_bits(unsigned n, void __iomem *reg)
{
	__raw_writel(__raw_readl(reg) & ~n, reg);
}
...
0 means input
*/
	*val = (__raw_readl(GPIO_CONFIG(*id)) & BIT(GPIO_OE_BIT))>>GPIO_OE_BIT;
	mb();
	return 0;
}
...
static int gpio_debug_level_set(void *data, u64 val)
{
	...
	spin_lock_irqsave(&tlmm_lock, irq_flags);
	__raw_writel(val ? BIT(GPIO_OUT_BIT) : 0, GPIO_IN_OUT(*id));
	mb();
	spin_unlock_irqrestore(&tlmm_lock, irq_flags);
...
{
	int *id = data;
	int dir = (__raw_readl(GPIO_CONFIG(*id)) & BIT(GPIO_OE_BIT))>>GPIO_OE_BIT;

	if (dir)
		*val = (__raw_readl(GPIO_IN_OUT(*id)) & BIT(GPIO_OUT_BIT))>>GPIO_OUT_BIT;
	else
		*val = __raw_readl(GPIO_IN_OUT(*id)) & BIT(GPIO_IN_BIT);
	mb();
	return 0;
}
...
static int gpio_debug_drv_set(void *data, u64 val)
{
	...
	spin_lock_irqsave(&tlmm_lock, irq_flags);
	//clear it first
	clr_gpio_bits((0x7 << GPIO_DRV_BIT),GPIO_CONFIG(*id));
	mb();
...
static int gpio_debug_func_sel_set(void *data, u64 val)
{
	...
	spin_lock_irqsave(&tlmm_lock, irq_flags);
	//clear it first
	clr_gpio_bits((0xf << GPIO_FUNC_SEL_BIT), GPIO_CONFIG(*id));
	mb();
...
static int gpio_debug_func_sel_get(void *data, u64 *val)
{
	...
	*val = (__raw_readl(GPIO_CONFIG(*id)) >> GPIO_FUNC_SEL_BIT) & 0xf;
	mb();
	return 0;
}
...
static int gpio_debug_pull_set(void *data, u64 val)
{
	...
	spin_lock_irqsave(&tlmm_lock, irq_flags);
	//clear it first,bit[1:0]
    clr_gpio_bits((0x3), GPIO_CONFIG(*id));
    mb();	
...
static int gpio_debug_pull_get(void *data, u64 *val)
{
	...
	*val = __raw_readl(GPIO_CONFIG(*id)) & 0x3;
	mb();
	return 0;
}
...
static int gpio_debug_int_enable_get(void *data, u64 *val)
{
	...
	*val = __raw_readl(GPIO_INTR_CFG(*id)) & 0x1;
	mb();
	return 0;
}
...
static int gpio_debug_int_owner_set(void *data, u64 val)
{
	...
	if (val)
		__raw_writel(TARGET_PROC_SCORPION, GPIO_INTR_CFG_SU(*id));
	else
		__raw_writel(TARGET_PROC_NONE, GPIO_INTR_CFG_SU(*id));
	mb();
	spin_unlock_irqrestore(&tlmm_lock, irq_flags);
...
static int gpio_debug_int_owner_get(void *data, u64 *val)
{
	...
	*val = __raw_readl(GPIO_INTR_CFG_SU(*id)) & 0x7;
	mb();
	return 0;
}
...
static int gpio_debug_int_type_get(void *data, u64 *val)
{
	...
	*val = (__raw_readl(GPIO_INTR_CFG(*id))>>0x1) & 0x3;
	mb();
	return 0;
}
