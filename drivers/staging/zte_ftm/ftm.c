...
static ssize_t store_fusion(struct sys_device *dev, struct sysdev_attribute *attr, const char *buf, size_t buf_sz)
{
	...
	int32_t fusion_type;
	int32_t fusion_offset = (int32_t)&(((struct boot_shared_imem_cookie_type*)0)->fusion_info);
	
	sscanf(buf, "%d", &fusion_type);

	pr_info("[gary]%s: fusion_type: %d\n", __func__, fusion_type);
	...
	{
		pr_err("%s: boot_shared_imem_cookie_ptr is NULL!\n", __func__);	
		return -EFAULT;
	}

#if 1
	pr_info("[gary] offset = 0x%x \n", fusion_offset);
	__raw_writel(FUSION_MAGIC_NUM_1, MSM_IMEM_BASE+fusion_offset);
	__raw_writel(FUSION_MAGIC_NUM_2, MSM_IMEM_BASE+fusion_offset+0x04);
	__raw_writel(fusion_type, MSM_IMEM_BASE+fusion_offset+0x08);
	__raw_writel(QFPROM_STATUS_UNKOWN, MSM_IMEM_BASE+fusion_offset+0x0C);
	mb();		
#else
	pr_err("[gary]%s: ftm_fusion_info_ptr: %p\n", __func__, ftm_fusion_info_ptr);	
	ftm_fusion_info_ptr->magic_1 = FUSION_MAGIC_NUM_1;
	ftm_fusion_info_ptr->magic_2 = FUSION_MAGIC_NUM_2;
	ftm_fusion_info_ptr->opt = fusion_type;
	ftm_fusion_info_ptr->status = QFPROM_STATUS_UNKOWN;
	boot_shared_imem_cookie_ptr->efs_recovery_flag= 0xDDCC;
	mb();
#endif
	return buf_sz;
}
...
static int32_t ftm_register_sysdev(struct sys_device *sysdev)
{
	int32_t ret;
	int32_t i;
	pr_info("%s: ftm_register_sysdev enter\n", __func__);

	ret = sysdev_class_register(&ftm_sysdev_class);
	if (ret)
	{
		pr_err("sysdev_class_register fail\n");
		return ret;
	}
	...
	for (i = 0; i < ARRAY_SIZE(ftm_attrs); i++)
	{
		...
		if (ret)
		{
			pr_err("sysdev_create_file fail\n");
			goto ftm_fail;
		}
	}
...
static int32_t __init ftm_init(void)
{
	...
	if (ret)
	{
		pr_err("%s: platform_driver_probe fail e\n", __func__);
	    return ret;
	}
